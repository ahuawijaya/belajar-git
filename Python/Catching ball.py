...
class bar:
    def __init__(self,canvas,x1,y1,x2,y2):
        ...
        self.y2=y2
        self.canvas=canvas
        self.ball=canvas.create_oval(self.x1,self.y1,self.x2,self.y2, fill = 'yellow', tags='dot2')
...
def ball_set():
    ...
    limit=0
    value=randint(0,570)
    balll=Ball(canvas,value,20,value+30,50)
    ball.move_ball()
def score_board():
    root2=Tk()
    ...
    button3.pack()
    button4=Button(canvas2,text='EXIT',bg='red',command=lambda:exit_handler(root2))
    button4.pack()
